...
GLuint Utils::loadTexture(const char *texImagePath) {	
	...
	if( image.empty() ){
		...
		cv::Mat img_tr;
		flip(image, img_tr, 0); //flip around the x-axis, 1,0,-1 

		glTexImage2D(GL_TEXTURE_2D,         // Type of texture
						0,                   // Pyramid level (for mip-mapping) - 0 is the top level
						GL_RGB,              // Internal colour format to convert to
						img_tr.cols,          // Image width  i.e. 640 for Kinect in standard mode
						img_tr.rows,          // Image height i.e. 480 for Kinect in standard mode
						0,                   // Border width in pixels (can either be 1 or 0)
						GL_BGR,              // Input image format (i.e. GL_RGB, GL_RGBA, GL_BGR etc.)
						GL_UNSIGNED_BYTE,    // Image data type
						img_tr.ptr());        // The actual image data itself
