...
void main (void) //gl_in, gl_Position, gl_TessCoord, built-in variables
...
//	gl_Position = mvp_matrix * vec4(u,0,v,1);               // shows original grid (pick one)
	...
	vec2 tc1 = mix(tcs_out[0], tcs_out[3], u);
	vec2 tc2 = mix(tcs_out[12], tcs_out[15], u);
	vec2 tc = mix(tc2, tc1, v);
	tes_out = tc;
}
